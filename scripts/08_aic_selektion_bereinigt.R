...
#                wurden bereits ausgeführt (Objekte "Dataset_bereinigt" und
...
modell_null_bereinigt <- lm(Arbeitszufriedenheit ~ 1, data = Dataset_bereinigt)
...
for (k in seq_along(praediktoren)) {
  ...
  for (komb in kombinationen) {
    ...
    alle_modelle[[name]] <- lm(formel, data = Dataset_bereinigt)
  }
}
